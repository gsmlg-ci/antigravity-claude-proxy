...
function loadConfig() {
    try {
        ...
        if (process.env.MODEL_ALIASES) {
            try {
                const parsed = JSON.parse(process.env.MODEL_ALIASES);
                if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
                    config.modelAliases = parsed;
                }
            } catch {
                config.modelAliases = {};
            }
        }
        if (process.env.AI_CREDIT_OVERAGES) config.aiCreditOverages = process.env.AI_CREDIT_OVERAGES;
