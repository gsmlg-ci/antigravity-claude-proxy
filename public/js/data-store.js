...
document.addEventListener('alpine:init', () => {
    Alpine.store('data', {
        ...
        modelAliases: {},
        aiCreditOverages: { mode: 'default', forwarded: false, forwardingMode: 'visibility-only', note: '' },
            ...
            try {
                ...
                if (cached) {
                    ...
                    if (data.accounts && data.models) {
                        ...
                        this.modelAliases = data.modelAliases || {};
                        this.aiCreditOverages = data.aiCreditOverages || this.aiCreditOverages;
        ...
        saveToCache() {
            try {
                const cacheData = {
                    ...
                    aiCreditOverages: this.aiCreditOverages,
        ...
        getModelAliasTarget(modelId) {
            return this.modelAliases?.[modelId] || '';
        },

        getModelDisplayLabel(modelId) {
            const target = this.getModelAliasTarget(modelId);
            return target ? `${modelId} -> ${target}` : modelId;
        },
